...
class Koan10 extends GroovyTestCase {
    ...
    void test01_XmlSlurpersReader() {
        ...
        def movieCount
        // ------------ START EDITING HERE ----------------------
        def movies = new XmlSlurper().parse('src/test/groovy/org/groovykoans/koan10/movies.xml').movie
        ...
        // ------------ START EDITING HERE ----------------------
        def filteredNodes = movies.title.findAll {
            it.text().toLowerCase().contains("the")
        }
        moviesWithThe = filteredNodes.collect {it.text()}
        // ------------ STOP EDITING HERE  ----------------------
        ...
        // ------------ START EDITING HERE ----------------------
        movieIdsGreaterThan5 = movies.findAll {
            it.@id.toInteger() > 5
        }.size()
        // ------------ STOP EDITING HERE  ----------------------
    ...
    void test04_XmlMarkupBuilder2() {
        ...
        def writer = new StringWriter()
        new MarkupBuilder(writer).movies {
            movies.each {
    ...
    void test06_AntBuilderChecksum() {
        ...
        // ------------ START EDITING HERE ----------------------
        def ant = new AntBuilder()
        ant.checksum(file: "${baseDir}/movies.xml", property: 'sum')
        actualChecksum = ant.project.properties.sum
        // ------------ STOP EDITING HERE  ----------------------
